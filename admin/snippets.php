...
$slugs = array_map(function($p) { return $p['slug']; }, $pages);
$pageList = $slugs;
$message = '';
$error = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (!bitsy_admin_check_csrf($_POST['csrf'] ?? '')) { http_response_code(403); exit('Invalid request.'); }
  if (isset($_POST['toggle'])) {
    bitsy_admin_toggle_snippet($_POST['toggle']);
    header('Location: snippets.php?msg=1'); exit;
  } elseif (isset($_POST['delete'])) {
    bitsy_admin_delete_snippet($_POST['delete']);
    header('Location: snippets.php?msg=2'); exit;
  } elseif (isset($_POST['save'])) {
    $ok = bitsy_admin_save_snippet($_POST);
    if ($ok) { header('Location: snippets.php?msg=3'); exit; }
...
<aside class="sidebar">
  <a class="brand" href="index.php"><img class="brand-mark" src="../assets/img/bitsy-favicon.png" alt="Bitsy AV"><span>Bitsy <small>Admin</small></span></a>
  <p class="nav-label">Workspace</p>
  <nav class="side-nav">
    <a href="index.php"><span></span>Dashboard</a>
    <a href="pages.php"><span></span>Pages<?php if ($navCounts['pages'] > 0): ?> <b><?php echo $navCounts['pages']; ?></b><?php endif; ?></a>
    <a href="posts.php"><span></span>Posts<?php if ($navCounts['posts'] > 0): ?> <b><?php echo $navCounts['posts']; ?></b><?php endif; ?></a>
    <a href="categories.php"><span></span>Categories<?php if ($navCounts['categories'] > 0): ?> <b><?php echo $navCounts['categories']; ?></b><?php endif; ?></a>
    <a href="media.php"><span></span>Media</a>
    <a href="leads.php"><span></span>Enquiries<?php if ($navCounts['leads'] > 0): ?> <b><?php echo $navCounts['leads']; ?></b><?php endif; ?></a>
  </nav>
...
<div class="page-toolbar">
  <div>
    <p class="muted"><?php echo $edit ? 'Editing: ' . bitsy_admin_escape($edit['name']) : 'Create a new code snippet to inject into your pages.'; ?></p>
  </div>
  ...
  <form method="post">
    ...
    <div class="form-grid">
      <div class="form-col">
        ...
        <input id="f-name" name="name" type="text" required placeholder="e.g. Google Analytics, Schema, Chat Widget" value="<?php echo bitsy_admin_escape($edit['name'] ?? ''); ?>">
      </div>
      <div class="form-col">
        <label for="f-priority">Priority</label>
        <input id="f-priority" name="priority" type="number" min="1" max="999" value="<?php echo bitsy_admin_escape($edit['priority'] ?? '10'); ?>">
    ...
    <div class="form-grid">
      ...
      <div class="form-col" id="slug-picker" style="<?php echo ($edit['page_scope'] ?? 'all') === 'specific' ? '' : 'display:none'; ?>">
        <label for="f-slugs">Page slugs (comma-separated)</label>
        <input id="f-slugs" name="page_slugs" type="text" placeholder="e.g. index, about-us, contact-us" value="<?php echo bitsy_admin_escape($edit['page_slugs'] ?? ''); ?>">
        <small>Available: <?php echo implode(', ', array_slice($pageList, 0, 10)); ?><?php echo count($pageList) > 10 ? '...' : ''; ?></small>
      </div>
    </div>
    ...
    <div class="form-col">
      ...
      <textarea id="f-content" name="content" rows="16" required class="code-editor" placeholder="Paste your code here..."><?php echo htmlspecialchars($edit['content'] ?? '', ENT_QUOTES, 'UTF-8'); ?></textarea>
    </div>

    <div class="form-actions">
      <a class="button secondary" href="snippets.php">Cancel</a>
      <button class="button primary" type="submit"><?php echo $edit ? 'Save changes' : 'Create snippet'; ?></button>
    </div>
  </form>
</section>
...
<div class="page-toolbar">
  <div>
    <p class="muted">Inject custom HTML, JavaScript, CSS, PHP, or Schema code into any page. Similar to the HFCM plugin.</p>
  </div>
...
<section class="panel">
  <div class="empty-content">
    ...
    <p>Add your first snippet to inject tracking code, schema markup, chat widgets, or any custom code into your pages.</p>
    ...
    <table>
      ...
      <?php foreach ($snippets as $s): ?>
        <tr class="<?php echo empty($s['enabled']) ? 'row-disabled' : ''; ?>">
          ...
          <td><span class="location-badge loc-<?php echo $s['location']; ?>"><?php echo $locations[$s['location']]; ?></span></td>
          <td><span class="type-badge type-<?php echo $s['content_type']; ?>"><?php echo strtoupper($s['content_type']); ?></span></td>
          <td><?php echo $scopeOptions[$s['page_scope']] ?? $s['page_scope']; ?></td>
          <td><?php echo (int) $s['priority']; ?></td>
          <td>
            <form method="post" class="inline-form">
              <input type="hidden" name="csrf" value="<?php echo bitsy_admin_escape(bitsy_admin_csrf()); ?>">
              <button type="submit" name="toggle" value="<?php echo (int) $s['id']; ?>" class="status-link <?php echo $s['enabled'] ? 'status-on' : 'status-off'; ?>">
          ...
          <td class="actions-cell">
            ...
            <form method="post" class="inline-form" onsubmit="return confirm('Delete this snippet?');">
              <input type="hidden" name="csrf" value="<?php echo bitsy_admin_escape(bitsy_admin_csrf()); ?>">
              <button class="delete-link" type="submit" name="delete" value="<?php echo (int) $s['id']; ?>">Delete</button>
            </form>
          </td>
        </tr>
      <?php endforeach; ?>
