...
if (isset($_GET['delete'])) {
  if (!bitsy_admin_check_csrf(isset($_GET['csrf']) ? $_GET['csrf'] : '')) {
    http_response_code(403);
    exit('Invalid request.');
  }
  $id = (int) $_GET['delete'];
  if (bitsy_cms_delete_post($id)) {
...
<body>
  <div class="admin-shell">
    <aside class="sidebar"><a class="brand" href="index.php"><img class="brand-mark"
      ...
      <nav class="side-nav"><a href="index.php"><span></span>Dashboard</a><a
          href="pages.php"><span></span>Pages<?php if ($navCounts['pages'] > 0): ?>
            <b><?php echo $navCounts['pages']; ?></b><?php endif; ?></a><a class="active"
          href="posts.php"><span></span>Posts</a><a
    ...
    <div class="admin-content">
      <header class="topbar">
        <div>
          <p class="eyebrow">CONTENT / POSTS</p>
          <h1>Posts</h1>
        </div><a class="avatar" href="logout.php">A</a>
      </header>
      <main class="admin-main">
        <div class="page-toolbar">
          <p class="muted">Articles and buyer guides published on the blog.</p>
          <?php if (!$isCreating): ?><a class="button primary" href="posts.php?new=1">+ New post</a><?php else: ?><a
              class="button secondary" href="posts.php">Cancel</a><?php endif; ?>
        </div>
        ...
        <?php if ($isCreating): ?>
          <section class="panel" style="padding:28px 30px">
            ...
            <p class="muted" style="margin:0 0 20px">
              <?php echo $isEditing ? 'Update the fields below and save.' : 'Fill in the fields and publish when ready.'; ?>
            </p>
            <form method="post" class="post-form">
              ...
              <input type="text" id="p-title" name="title"
                value="<?php echo bitsy_admin_escape(isset($edit['title']) ? $edit['title'] : ''); ?>" required
                placeholder="How to size a video wall for your room">
              <div class="row">
                <div><label for="p-category">Category</label><select id="p-category" name="category_id">
                    <option value="">No category</option><?php foreach ($categories as $cat): ?>
                      <option value="<?php echo (int) $cat['id']; ?>" <?php echo (isset($edit['category_id']) && (int) $edit['category_id'] === (int) $cat['id']) ? ' selected' : ''; ?>>
                        <?php echo bitsy_admin_escape($cat['name']); ?></option><?php endforeach; ?>
                  </select><small><a href="categories.php">Manage categories</a></small></div>
                <div><label for="p-slug">Slug</label>
                  <input type="text" id="p-slug" name="slug"
                    value="<?php echo bitsy_admin_escape(isset($edit['slug']) ? $edit['slug'] : ''); ?>"
              ...
              <div class="row">
                <div><label for="p-status">Status</label><select id="p-status" name="status">
                    <option value="draft" <?php echo (isset($edit['status']) && $edit['status'] === 'draft') ? ' selected' : ''; ?>>Draft</option>
                    <option value="published" <?php echo (isset($edit['status']) && $edit['status'] === 'published') ? ' selected' : ''; ?>>Published</option>
                    <option value="private" <?php echo (isset($edit['status']) && $edit['status'] === 'private') ? ' selected' : ''; ?>>Private</option>
                  </select></div>
                <div><label for="p-published">Published at</label><input type="text" id="p-published" name="published_at"
                    value="<?php echo bitsy_admin_escape(isset($edit['published_at']) ? $edit['published_at'] : ''); ?>"
                    placeholder="2024-06-15 10:00:00"></div>
              </div>
              <label for="p-excerpt">Excerpt</label>
              <textarea id="p-excerpt" name="excerpt"
                placeholder="Short description shown on the blog listing card."><?php echo bitsy_admin_escape(isset($edit['excerpt']) ? $edit['excerpt'] : ''); ?></textarea>
              <label for="p-content">Content (HTML)</label>
              <textarea id="p-content" name="content" class="content-area"
                placeholder="Full article body — HTML is supported."><?php echo bitsy_admin_escape(isset($edit['content']) ? $edit['content'] : ''); ?></textarea>
              <label for="p-meta">Meta description (SEO)</label>
              <input type="text" id="p-meta" name="meta_description"
                value="<?php echo bitsy_admin_escape(isset($edit['meta_description']) ? $edit['meta_description'] : ''); ?>"
                placeholder="Short sentence for search engine results.">
              <label for="p-image">Featured image path</label>
              <input type="text" id="p-image" name="featured_image"
                value="<?php echo bitsy_admin_escape(isset($edit['featured_image']) ? $edit['featured_image'] : ''); ?>"
                placeholder="assets/img/Audio-video-solution.webp">
              <div style="margin-top:20px;display:flex;gap:10px">
                <button class="button primary"
                  type="submit"><?php echo $isEditing ? 'Save post' : 'Create post'; ?></button>
        ...
        <section class="panel page-table">
          <div class="table-tools"><strong>All posts <span
                class="muted"><?php echo count($posts); ?></span></strong><span
              class="table-note"><?php echo count(array_filter($posts, function ($p) {
              ...
              <table class="post-table">
                ...
                <tbody>
                  <?php foreach ($posts as $post): ?>
                    <tr>
                      <td><strong><?php echo bitsy_admin_escape($post['title']); ?></strong>
                        <div class="post-excerpt">
                          <?php echo bitsy_admin_escape(mb_strimwidth($post['excerpt'], 0, 80, '…')); ?></div>
                      </td>
                      <td style="font-size:13px;color:var(--muted)">
                        <?php echo $post['category'] ? bitsy_admin_escape($post['category']) : '—'; ?></td>
                      <td><span
                          class="status-badge <?php echo bitsy_admin_escape($post['status']); ?>"><?php echo bitsy_admin_escape($post['status']); ?></span>
                      </td>
                      <td><code style="font-size:12px"><?php echo bitsy_admin_escape($post['slug']); ?></code></td>
                      <td style="font-size:13px;color:var(--muted)">
                        <?php echo $post['published_at'] ? bitsy_admin_escape(date('j M Y', strtotime($post['published_at']))) : '—'; ?>
                      </td>
                      <td style="white-space:nowrap">
                        <a class="button secondary" style="padding:5px 10px;font-size:12px"
                          href="posts.php?edit=<?php echo (int) $post['id']; ?>">Edit</a>
                        <a class="button secondary" style="padding:5px 10px;font-size:12px"
                          href="posts.php?toggle=<?php echo (int) $post['id']; ?>&csrf=<?php echo bitsy_admin_escape(bitsy_admin_csrf()); ?>"><?php echo $post['status'] === 'published' ? 'Unpublish' : 'Publish'; ?></a>
                        <a class="button" style="padding:5px 10px;font-size:12px;background:#fde8e8;color:#a33"
                          href="posts.php?delete=<?php echo (int) $post['id']; ?>&csrf=<?php echo bitsy_admin_escape(bitsy_admin_csrf()); ?>"
